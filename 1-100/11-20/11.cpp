...
int main()
{
	...
	constexpr auto dims = 20 + (range - 1) * 2;

	constexpr long testArray[dims][dims] = {
	...
	long muls[dims * dims * 4];	//	4 directions per digit
	std::fill(std::begin(muls), std::end(muls), 1);
	...
	for (auto i = range - 1; i < dims - range - 1; i++)
	{
		for (auto j = range - 1; j < dims - range - 1; j++, i1++, i2++, i3++, i4++)
		{
			for (auto k = 0; k < range; k++)
			{
				muls[i1] *= testArray[i + k][j];
				muls[i2] *= testArray[i][j + k];
				muls[i3] *= testArray[i + k][j + k];
				muls[i4] *= testArray[i - k][j + k - range + 1];
			}
		}
	}

	auto max = std::max_element(std::begin(muls), std::end(muls));
	auto t2 = std::chrono::high_resolution_clock::now();

	std::chrono::duration<double, std::milli> fp_ms = t2 - t1;
	std::cout << "Max = " << *max << " time = " << fp_ms.count() << std::endl;
